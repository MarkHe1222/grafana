...
import CorsWorkerPlugin from '../webpack/plugins/CorsWorkerPlugin.ts';

// jsdom omits setImmediate, which webpack's AsyncQueue needs. This suite can't just use the
// node environment because the repo-wide jest setup files require `window`.
Object.assign(globalThis, { setImmediate, clearImmediate });

const OUTPUT_PATH = '/dist';
const LITERAL_PUBLIC_PATH = '/public/build/';

// The generated worker runtime. Kept as literals so a change in either the plugin or in
// webpack's own AutoPublicPathRuntimeModule fails the test rather than passing silently.
...
function createConfig(publicPath: string): Configuration {
  return {
    context: path.join(__dirname, '__fixtures__', 'cors-worker'),
    entry: './index.js',
    mode: 'development',
    devtool: false,
    // Explicit target so the output doesn't depend on the repo's .browserslistrc.
    target: ['web', 'es2022'],
    output: {
      path: OUTPUT_PATH,
      publicPath,
