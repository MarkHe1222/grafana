...
export const manifestPluginOptions: Partial<Options> = {
  ...
  // A literal prefix, not `true`: `true` reads output.publicPath, which is 'auto' and
  // resolves to no prefix at all. The backend renders these paths verbatim into
  // <script src> and <link href>, where a bare filename breaks every non-root route.
