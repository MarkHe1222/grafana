...
export default (env: Env = {}): Configuration => ({
  ...
  output: {
    ...
    // Derived at runtime from the URL of the script that loads the runtime chunk, so
    // chunks, workers and assets resolve under a CDN or a sub-path without the app
    // reassigning __webpack_public_path__. The assets manifest needs a literal prefix
    // instead — see scripts/webpack/plugins/assetsManifest.ts.
    publicPath: 'auto',
  },
  resolve: {
